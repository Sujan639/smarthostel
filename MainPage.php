...
<head>
  ...
  <style>
    @media (max-width: 600px) .ItemsB>div {
      min-height: 370px;
      max-height: 370px;
    }

    div.ItemsB:nth-child(odd) {
      padding-left: 5px;
    }
    ...
    @media (max-width: 992px) .prod-img-wrap a {
      height: 200px;
    }

    #ItemsBody_pnlCategoryBrowse1 .product-img {
      margin-bottom: 20px;
      max-height: 200px;
      text-align: center;
    }

    #ItemsBody_pnlCategoryBrowse1 .product-img img {
    ...
    @media (max-width: 600px) .prod-img-wrap a {
      max-height: 130px;
...
?>
      ...
      <form class="form-inline my-2 my-lg-0" method="POST" action="searchengine1.php">
        <input type="text" placeholder="search By Location, Price, HstelName " name="search"
          class="form-control mr-sm-2" name placeholder="Search" aria-label="Search">
...
while($row=mysqli_fetch_assoc($retval))
{
  ...
  echo'
      ...
      <span style="color:black;"> HostelType:  '.$row['HostelType'].'</span>
          ...
          <!--ItemsBody_pnlBestSellers-->
        </div>
        <!--ItemsBody_pnlCategoryBrowse1-->
      </div>
      <!--div content end-->
