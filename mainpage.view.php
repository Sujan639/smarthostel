...
if(! $retval)
{
  ...
  $sql1=mysqli cubrid_fetch_assoc($con,$sql);
...
$n = 3; 
  ...
  for ($i = 0; $i < $n; $i++) 
   ...
   $keyword = $sql = array();
  $uniqueMergedkeyword = array_merge($a, $b);
  foreach ($keyword as $keyword=>$val) $uniquekeyword[$keyword] = $val;
  foreach ($sql as $sql=>$val) $uniquesql[$sql] = $val;
  $a=0;
  $b=0;
  $xArray=array();
  $yArray=array();
  $id=1;
  foreach ($uniqueMergedkeywords as $keyword=>$v) {
    $xArray[$id] = isset($keyword[$a]) ?  $a[$keyword]: 0;
    $yArray[$id] = isset($keyword[$b]) ?  $b[$sql]: 0;
    $a+=$xArray[$id]*$xArray[$id];
    $b+=$yArray[$id]*$yArray[$id];
    $id++;
    }
  $a=sqrt($a);
  $b=sqrt($b);
